...
\begin{frame}{2D Analog: Four Corners + Four Edges}
    ...
    \begin{center}
        \includegraphics[width=0.7\textwidth]{figures/fig_2d_ctmrg_structure.pdf}
    \end{center}
    ...
    \begin{block}{Key: What is $a$?}
        $a$ is a \highlight{rank-4 local tensor} --- for computing $Z$: $a = W$; for $\langle \sigma \rangle$: $a = \sigma \cdot W$.
    \end{block}
\end{frame}
...
\begin{frame}{The Plaquette Tensor: From Bonds to Faces}
    ...
    \begin{columns}
        \begin{column}{0.5\textwidth}
            ...
            Bond-based $T$ (rank-2) $\to$ Plaquette-based $W$ (rank-4)
            ...
            \begin{center}
                \fbox{\parbox{0.8\textwidth}{
                    \centering
                    \vspace{2cm}
                    \textit{[Figure: 4 bonds around a plaquette $\to$ $W$]}
                    \vspace{2cm}
                }}
            \end{center}
        \end{column}
        ...
        \begin{column}{0.5\textwidth}
            ...
            Combine 4 bond tensors $T$ around a face:
            \[
            W_{u,d,l,r} = \sum_{\sigma} T_{\sigma,u} T_{\sigma,d} T_{\sigma,l} T_{\sigma,r}
            \]
            
            where $T_{\sigma, \sigma'} = e^{\frac{\beta J}{2} \sigma \sigma'}$
            
            \vspace{0.3cm}
            
            \begin{block}{Why Plaquette?}
                $W$ is the \highlight{rank-4 local tensor} used in CTMRG.
                
                Each $W$ encodes one face of the tilted lattice.
            \end{block}
        \end{column}
    \end{columns}
\end{frame}
