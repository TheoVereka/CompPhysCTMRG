...
\begin{frame}{2D: The Transfer Matrix Becomes a \highlight{Network}}
    ...
    \begin{columns}
        ...
        \begin{column}{0.45\textwidth}
            ...
            \textbf{2D:} $T$ still $2 \times 2$ {\tiny (from now on }$d \times d$\\{\tiny in order to distinguish physical dimension $D=2$) }\\on each \highlight{bond},
    ...
    \begin{alertblock}{The Fundamental Problem}
        \highlight{No natural contraction order!} 2D tensor network cannot be reduced to a simple trace.
    \end{alertblock}
\end{frame}
...
\begin{frame}{One Approach: Row-to-Row Transfer (DMRG-style)}
    ...
    \textbf{Idea:} Group one row of $L$ spins $\to$ treat as a ``super-spin'' with $d^L$ states.
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{center}
                \fbox{\parbox{0.95\textwidth}{
                    \centering
                    \vspace{1.8cm}
                    \textit{[Figure: Row $\to$ super-spin, $T_{\text{row}} \in \mathbb{R}^{d^L \times d^L}$]}
                    \vspace{1.8cm}
                }}
            \end{center}
            
            Then $Z = \text{Tr}(T_{\text{row}}^L)$ looks like 1D!
        \end{column}
...
\begin{frame}{2D Analog: Four Corners + Four Edges}
    ...
    \begin{block}{Key: What is $a$?}
        $a$ is a \highlight{rank-4 tensor} --- for computing $Z$: $a = W$ \textit{plaquette tensor}; for $\langle \sigma_1 \rangle$: $a = s_1 W_{s_1s_2s_3s_4}$, etc.
    \end{block}
\end{frame}
...
\begin{frame}{The Plaquette Tensor: From Bonds to Faces}
    ...
    \begin{center}
        \includegraphics[width=0.85\textwidth]{figures/fig_plaquette_tensor.pdf}
    \end{center}
    
    \vspace{-0.2cm}
    
    \begin{block}{Key: Simple scalar multiplication of 4 bond weights}
        \vspace{-0.2cm}
        \[
        W_{s_1,s_2,s_3,s_4} = T_{s_1,s_2} \cdot T_{s_2,s_3} \cdot T_{s_3,s_4} \cdot T_{s_4,s_1}
        \quad\text{where}\quad
        T_{s,s'} = e^{\frac{\beta J}{2} s \cdot s'}
        \]
    \end{block}
\end{frame}
...
\begin{frame}{CTMRG Iteration: Step 2 --- Truncate}
    ...
    \begin{center}
        \includegraphics[width=0.83\textwidth]{figures/fig_ctmrg_truncate.pdf}
    \end{center}
\end{frame}
...
\begin{frame}{Summary: The Core of CTMRG}
    ...
    \begin{center}
    	\includegraphics[width=0.95\textwidth]{figures/aaa.png}
    \end{center}
\end{frame}
...
\begin{frame}
    \begin{center}
        ...
        \\
        \vspace{1cm}
        \small also a small thanks to Claude Opus that assists me generating \textit{all} the schemas and teaching me \texttt{beamer}.
    \end{center}
\end{frame}
...
% ------------------------------------------------------------
% Appendix A.6: Original CTMRG Results
% ------------------------------------------------------------
\begin{frame}{Appendix: Original CTMRG Results --- 2D Ising Model}
    
    \begin{center}
        \textbf{From T. Nishino \& K. Okunishi, \textit{J. Phys. Soc. Jpn.} \textbf{65}, 891 (1996)}
    \end{center}
    
    
    \begin{center}
        \includegraphics[width=0.5\textwidth]{figures/fig_nishino_result.png}
    \end{center}
    
    \vspace{0.2cm}
    
    \begin{block}{Key Result}
        Local energy $E(T)$ computed via CTMRG converges rapidly with iterations $N$. 
        The critical temperature $T_c$ (inset) can be accurately determined from finite-size scaling.
    \end{block}
\end{frame}

% ------------------------------------------------------------
% Appendix A.7: CTMRG on Other Lattices
% ------------------------------------------------------------
\begin{frame}{Appendix: CTMRG on Other 2D Lattices --- Ruby Lattice}
    
    \vspace{-0.2cm}
    \begin{center}
        \textbf{Extension to non-square lattices}
    \end{center}
    
    
    \begin{center}
        \includegraphics[width=0.5\textwidth]{figures/fig_ruby_lattice.png}
    \end{center}
    
    
    \begin{block}{Generalization}
        CTMRG can be adapted to various 2D lattice geometries (honeycomb, triangular, Ruby, etc.) 
        by modifying the corner/edge tensor definitions while preserving the RG iteration structure.
    \end{block}
\end{frame}
